...
async function highlightThisMessage(user, message, extra) {
  ...
  if(isTTSEnabled) {
    ...
    const speakUrl = await fetch(`${TTS_BASE}?${qs}`)
        .then(response => response.json())
        .then(data => data['speak_url']);

    const speak = await fetch(speakUrl);
...
if(channel) {
  ...
  fetch(
    ...
    { headers: { "Client-ID": "ucvrj4bho435lsb91wpv0tkrroqzp1", "Authorization": "Bearer vmk0e9oarug5b5mlil7wkxmv6yh8ww" } }
 )
  .then(r => r.json())
